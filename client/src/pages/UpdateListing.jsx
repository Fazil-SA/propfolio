...
const handleFormSubmit = async (e) => {
    ...
    try {
        setLoading(true);
        setError(false);
        axiosInstance.post(`/api/listing/update/${params.listingID}`, formData, {withCredentials: true})
        .then((res) => {
            setLoading(false);
  ...
  return (
    <main className="p-3 max-w-4xl mx-auto">
        <h1 className="text-center text-3xl font-semibold my-7">Update your listing</h1>
        <form onSubmit={handleFormSubmit} className="flex flex-col sm:flex-row gap-6">
            <div className="flex flex-col flex-1 gap-4">
                <input type="text" onChange={handleChange} value={formData.title} className="p-3 border rounded-lg" placeholder="Title" id="title" required/>
                <textarea type="text" onChange={handleChange} value={formData.description} className="p-3 border rounded-lg" placeholder="Description" id="description" required/>
                <input type="text" onChange={handleChange} value={formData.address} className="p-3 border rounded-lg" placeholder="Address" id="address" required/>
                <div className="flex flex-wrap gap-4">
                    ...
                    <div className="flex gap-2">
                        <input onChange={handleChange} className="w-5" checked={formData.parking} type="checkbox" id="parking"/>
                        <span>Parking Spot</span>
                    </div>
                    <div className="flex gap-2">
                        <input onChange={handleChange} className="w-5" checked={formData.furnished} type="checkbox" id="furnished"/>
                        <span>Furnished</span>
                    </div>
                    <div className="flex gap-2">
                        <input onChange={handleChange} className="w-5" checked={formData.offer} type="checkbox" id="offer"/>
                        <span>Offer</span>
                    </div>
                </div>
            ...
            <button disabled={loading || uploading} className="mt-1 w-full bg-slate-700 uppercase p-3 border rounded-lg text-white hover:opacity-95">{loading ? "Updating" : "Update Listing"}</button>
            </div>  
